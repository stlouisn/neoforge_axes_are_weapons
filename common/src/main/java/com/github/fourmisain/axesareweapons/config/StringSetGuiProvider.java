...
public class StringSetGuiProvider<T> implements GuiProvider {
  ...
  public List<AbstractConfigListEntry> get(String i13n, Field field, Object config, Object defaults, GuiRegistryAccess registry) {
    return Collections.singletonList(ConfigEntryBuilder.create().startStrList(Text.translatable(i13n), Utils.<Set<T>>getUnsafely(field, config).stream().map(Object::toString).collect(toList())).setDefaultValue(() -> Utils.<Set<?>>getUnsafely(field, defaults).stream().map(Object::toString).collect(toList())).setSaveConsumer(newValue -> Utils.setUnsafely(field, config, newValue.stream().map(constructor).collect(toSet()))).build());
  }
}